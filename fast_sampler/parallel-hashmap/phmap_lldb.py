...
class flat_hash_map_or_set:
    ...
    def summary(valobj, _):
        try:
            ...
            capacity = valobj.GetChildMemberWithName(
    ...
    def update(self):
        try:
            self.size_ = self.valobj.GetChildMemberWithName(
                'size_').GetValueAsUnsigned()
            self.capacity_ = self.valobj.GetChildMemberWithName(
                'capacity_').GetValueAsUnsigned()
...
class parallel_flat_or_node_map_or_set:
    ...
    def _get_size_and_capacity(valobj):
        try:
            ...
            m = parallel_flat_or_node_map_or_set.REGEX_EXTRACT_ARRAY_SIZE.match(
                sets.GetType().GetName())
            ...
            for idx in range(n_buckets):
                bucket = buckets.GetChildAtIndex(
                    idx).GetChildMemberWithName('set_')
                size += bucket.GetChildMemberWithName(
                    'size_').GetValueAsUnsigned()
                capacity += bucket.GetChildMemberWithName(
                    'capacity_').GetValueAsUnsigned()
    ...
    @staticmethod
    def summary(valobj, _):
        size, capacity, _ = parallel_flat_or_node_map_or_set._get_size_and_capacity(
            valobj)
        return f"size = {size} (capacity = {capacity})"
    ...
    def update(self):
        try:
            self.size_, self.capacity_, self.n_buckets_ = self._get_size_and_capacity(
                self.valobj)
            self.buckets = self.valobj.GetChildMemberWithName(
                'sets_').GetChildMemberWithName('_M_elems')
            bucket0 = self.buckets.GetChildAtIndex(
                0).GetChildMemberWithName('set_')
            self.slot_type = bucket0.GetChildMemberWithName(
                'slots_').GetType().GetPointeeType()
            self.slot_size = self.slot_type.GetByteSize()
        except BaseException as ex:
            print(f"{_get_function_name(self)} -> {ex}")
    ...
    def get_child_at_index(self, index):
        try:
            ...
            for idx in range(self.n_buckets_):
                bucket = self.buckets.GetChildAtIndex(
                    idx).GetChildMemberWithName('set_')
                size = bucket.GetChildMemberWithName(
                    "size_").GetValueAsUnsigned()
                if size:
