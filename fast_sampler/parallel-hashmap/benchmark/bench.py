        ...
        for program in programs:
            ...
            for attempt in range(best_out_of):
                proc = subprocess.Popen(
                    ['./build/'+program, str(nkeys), benchtype], stdout=subprocess.PIPE)
                ...
                ps_proc = subprocess.Popen(
                    ['ps up %d | tail -n1' % proc.pid], shell=True, stdout=subprocess.PIPE)
                ...
                if nbytes and runtime:  # otherwise it crashed
                    line = ','.join(
                        map(str, [benchtype, nkeys, program, nbytes, "%0.6f" % runtime]))
